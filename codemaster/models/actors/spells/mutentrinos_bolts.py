...
class MutentrinosBolt(ActorMagic):
    """Represents a Muten Ro neutrinos bolt.
    ...
    def __init__(self, x, y, game, name=None, is_from_player_shot=None,
                 ...
                 change_x=3.8, change_y=3.1):
        self.file_folder = BM_MAGIC_FOLDER
        self.file_name_key = 'im_mutentrinos_bolt'
...
class MutentrinosBoltB(MutentrinosBolt):
    """Represents a Muten Ro neutrinos bolt, of type B."""
    actor_type = ActorType.MUTENTRINOS_BOLT_B
    max_spells_on_target = 1
    power_min_to_use = {ActorType.MUTENTRINOS_BOLT_B.name: 18,
                        }
    power_consumption = {ActorType.MUTENTRINOS_BOLT_B.name: 14,
                         }

    def __init__(self, x, y, game, name=None, is_from_player_shot=None,
                 owner=None, target=None,
                 change_x=8.6, change_y=6.9):
        self.file_mid_prefix = '01'
        self.type = ActorType.MUTENTRINOS_BOLT_B
        self.stats = Stats()
        super().__init__(x, y, game, name=name,
                         is_from_player_shot=is_from_player_shot,
                         owner=owner, target=target,
                         change_x=change_x, change_y=change_y)
        self.power = self.power_total = 120
        self.color = 67, 0, 0
        self.color_external = 166, 10, 0
        self.color_mode = pg.BLEND_RGB_ADD
        self.size_rand = 3, 8
        self.size_ext_multiplier = 1.8


class MutentrinosBoltA(MutentrinosBolt):
    """Represents a Muten Ro neutrinos bolt, of type A."""
    actor_type = ActorType.MUTENTRINOS_BOLT_A
    max_spells_on_target = 1
    power_min_to_use = {ActorType.MUTENTRINOS_BOLT_A.name: 20,
                        }
    power_consumption = {ActorType.MUTENTRINOS_BOLT_A.name: 16,
                         }

    def __init__(self, x, y, game, name=None, is_from_player_shot=None,
                 owner=None, target=None,
                 change_x=9.2, change_y=7.7):
        self.file_mid_prefix = '01'
        self.type = ActorType.MUTENTRINOS_BOLT_A
        self.stats = Stats()
        ...
        self.power = self.power_total = 134
        self.color = 252, 255, 25
        self.color_external = 80, 152, 0
        self.color_mode = pg.BLEND_RGB_ADD
        self.size_rand = 4, 9
