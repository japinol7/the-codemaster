...
def test_pc_must_fall_if_no_platform_under_them(game):
    player = game.player
    player_orig_y = 554
...
@game_test(levels=[3], timeout=2)
def test_pc_must_not_fall_if_platform_under_them(game):
    player = game.player
    player_orig_y = 478
...
def test_pc_must_not_move_if_not_moving_platform(game):
    player = game.player
    player_orig_x = 450
...
@game_test(levels=[6], timeout=2)
def test_pc_must_not_move_if_not_moving_platform_scroll(game):
    player = game.player
    player_orig_x = 1600
    player.rect.x, player.rect.y = player_orig_x, 665
...
def test_pc_must_move_if_on_moving_platform_left(game):
    player = game.player
    player_orig_x = 450
    player.rect.x, player.rect.y = player_orig_x, 665
...
def test_pc_must_move_if_on_moving_platform_right(game):
    player = game.player
    player_orig_x = 360
    player.rect.x, player.rect.y = player_orig_x, 665
...
def test_pc_must_die_when_reaches_bottom_of_screen(game):
    player = game.player
