...
class TestPersistenceItems:
    """Tests persistence of the game state regarding the items
    present in each level when the game start.
    """

    @game_test(levels=[1, 2, 3, 4], starting_level=3, timeout=3.6)
    def test_persist_itens_2_levels_potions_n_get_life_rec(self, game):
    ...
    def test_persist_itens_2_levels_shot_n_get_1_potion(self, game):
        """Tests persistence of items for two visited levels:
