...
class Game:
    ...
    def _print_test_results(self):
        ...
        if self.tests_aborted:
            log.info(f"{self.tests_aborted} tests aborted by user")
        log.info(f"{self.tests_passed} tests passed of {self.tests_total}")
        if self.tests_passed < self.tests_total:
            log.warning("Some tests did not pass or were aborted by user.")
        log.info(f"{'-' * 50}")
        log.info(f"{'-' * 50}")
    ...
    def test_fetch_two_apples(self):
        ...
        if not self.aborted:
            if self.player.stats['apples'] != 2:
                log.info('Test not passed: Player did not fetch 2 apples.')
                return
            self.tests_passed += 1

    def test_big_jump_and_fetch_3_batteries_n_1_disk(self):
        log.info("Start test_big_jump")
        self.player.rect.x = 1502
        self.player.rect.y = 50
        self.player.stats['health'] = PLAYER_HEALTH_SUPER_HERO

        TextMsg.create(f"{IN_GAME_START_MSG}\nTest: test_big_jump",
                       self, time_in_secs=5)
        self._init_clock_timer(time_in_secs=6)
        ...
        if not self.aborted:
            if self.player.stats['batteries'] < 3 or self.player.stats['files_disks'] < 1:
                log.warning('Test not passed: Player did not fetch at least 3 batteries and 1 disk.')
                return
            self.tests_passed += 1
